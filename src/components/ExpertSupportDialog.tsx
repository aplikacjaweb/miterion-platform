...
export default function ExpertSupportDialog({ 
  trigger, 
  open: controlledOpen, 
  onOpenChange: controlledOnOpenChange, 
  initialTab = 'selection' 
}: ExpertSupportDialogProps) {
  ...
  const isClosed = controlledOpen !== undefined ? !controlledOpen : !internalOpen;
  ...
  const handleSelectTab = (tab: 'selection' | 'expert' | 'quote' | 'notsure') => {
    setActiveTab(tab);
    setCaptchaToken(null);
  };
...
`;
  ...
  return (
    <Dialog open={controlledOpen !== undefined ? controlledOpen : internalOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-[550px] overflow-y-auto max-h-[90vh] bg-white text-slate-900 p-6 rounded-2xl">
        ...
        {success ? (
          <div className="text-center py-12">
            ...
            <Button className="mt-6 bg-navy text-white hover:opacity-90 px-6 py-2 rounded-xl" onClick={() => handleOpenChange(false)}>Close</Button>
          </div>
        ) : activeTab === 'selection' ? (
          <>
            <DialogHeader>
          ...
          <form onSubmit={handleFormSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input required type="text" value={commonFields.fullName} onChange={e => setCommonFields({...commonFields, fullName: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-700 block mb-1">Work Email</label>
                <input required type="email" value={commonFields.workEmail} onChange={e => setCommonFields({...commonFields, workEmail: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900" />
              </div>
            </div>
            <div>
              <label className="text-xs font-bold text-gray-700 block mb-1">Company Name</label>
              <input required type="text" value={commonFields.companyName} onChange={e => setCommonFields({...commonFields, companyName: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900" />
            </div>
              ...
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required type="text" placeholder="e.g. Oncology, Rare disease" value={preFeasFields.indication} onChange={e => setPreFeasFields({...preFeasFields, indication: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-700 block mb-1">Phase</label>
                    <input required type="text" placeholder="e.g. Phase II, III" value={preFeasFields.phase} onChange={e => setPreFeasFields({...preFeasFields, phase: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900" />
                  </div>
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-700 block mb-1">Geography / Countries under consideration</label>
                  <input required type="text" value={preFeasFields.geography} onChange={e => setPreFeasFields({...preFeasFields, geography: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900" />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-700 block mb-1">What decision are you preparing?</label>
                  <input required type="text" placeholder="e.g. Country selection budget sign-off" value={preFeasFields.decision} onChange={e => setPreFeasFields({...preFeasFields, decision: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-bold text-gray-700 block mb-1">Timeline</label>
                    <input required type="text" placeholder="e.g. Next 3 months" value={preFeasFields.timeline} onChange={e => setPreFeasFields({...preFeasFields, timeline: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-700 block mb-1">Do you have a protocol synopsis?</label>
                    <select value={preFeasFields.protocolSynopsis} onChange={e => setPreFeasFields({...preFeasFields, protocolSynopsis: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900">
                ...
                <div>
                  ...
                  <textarea value={commonFields.additionalContext} onChange={e => setCommonFields({...commonFields, additionalContext: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy h-16 resize-none bg-white text-slate-900" />
                </div>
              </>
            )}
              ...
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={quoteFields.proposalType} onChange={e => setQuoteFields({...quoteFields, proposalType: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900">
                  ...
                  <div>
                    ...
                    <input required type="number" min="1" value={quoteFields.proposalCount} onChange={e => setQuoteFields({...quoteFields, proposalCount: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900" />
                  </div>
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-700 block mb-1">What decision are you preparing?</label>
                  <input required type="text" placeholder="e.g. Vendor selection board presentation" value={quoteFields.decision} onChange={e => setQuoteFields({...quoteFields, decision: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-bold text-gray-700 block mb-1">Target decision date</label>
                    <input required type="text" placeholder="e.g. Oct 15, 2026" value={quoteFields.targetDate} onChange={e => setQuoteFields({...quoteFields, targetDate: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-700 block mb-1">Do you need secure upload link?</label>
                    <select value={quoteFields.secureUpload} onChange={e => setQuoteFields({...quoteFields, secureUpload: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900">
                      <option value="Yes">Yes</option>
                      <option value="No">No</option>
                    </select>
                  </div>
            ...
            {activeTab === 'notsure' && (
              <>
                <div>
                  ...
                  <textarea required value={situationFields.decisionFacing} onChange={e => setSituationFields({...situationFields, decisionFacing: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy h-20 resize-none bg-white text-slate-900" />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-700 block mb-1">What would happen if this decision is wrong?</label>
                  <textarea required value={situationFields.riskIfWrong} onChange={e => setSituationFields({...situationFields, riskIfWrong: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy h-20 resize-none bg-white text-slate-900" />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-700 block mb-1">Timeline</label>
                  <input required type="text" placeholder="e.g. Immediate, critical path" value={situationFields.timeline} onChange={e => setSituationFields({...situationFields, timeline: e.target.value})} className="w-full text-sm border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:border-navy bg-white text-slate-900" />
                </div>
              </>
            )}
